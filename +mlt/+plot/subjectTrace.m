...
function markBadCallback(hObject, ~)
    ...
    h = msgbox('Click near bad beats. Press Enter when done.', 'Mark Bad Beats', 'modal');
...
function markMissingCallback(hObject, ~)
    ...
    h = msgbox('Click to add missing beats. Press Enter when done.', 'Mark Missing Beats', 'modal');
    uiwait(h); % Wait until user clicks OK
