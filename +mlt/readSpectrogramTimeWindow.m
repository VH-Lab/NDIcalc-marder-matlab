function [spectrogram_data, f, t_datetime, spectrogram_doc] = readSpectrogramTimeWindow(S, e, t0, t1)
...
%       READSPECTROGRAMTIMEWINDOW(S, E, T0, T1)
%   searches an NDI session or dataset S for 'spectrogram' type documents
%   that depend on the NDI element E. It then identifies which of these
...
%
%   Inputs:
%       S  - An ndi.session or ndi.dataset object. Must have a valid syncgraph.
%       E  - An ndi.element object.
...
%
%   Logic:
%       1. Finds all 'spectrogram' documents depending on element E.
%       2. Retrieves the epoch table for element E.
%       3. For each spectrogram document found (in order):
...
%       4. If loop completes without a match, returns empty outputs.
%
%   Assumptions: (Same as previous version)
%       - 'spectrogram' documents exist with necessary superclasses/properties.
%       - Data stored in 'spectrogram_results.ngrid'.
%       - Element epochtable has 'exp_global_time' entries with datenum t0_t1.
%       - Ngrid time coordinate vector is 'dev_local_time'.
%       - `mlt.readngrid` function is available.
%       - Session S has a valid `syncgraph`.
    ...
    query = q_type & q_depends; % [cite: 35]
    ...
    for i=1:numel(docs)
        ...
        try
            ...
            doc_epoch_id = current_doc.document_properties.epochid.epochid; % [cite: 7098]
            ...
            if ~isempty(et_entry_index)
                ...
                for k=1:numel(et_entry.epoch_clock)
                   if strcmpi(et_entry.epoch_clock{k}.type,'exp_global_time') % [cite: 6610]
                       clock_index = k;
                       break;
                   end
                end
                ...
                if clock_index > 0
                    ...
                    epoch_t0_t1 = et_entry.t0_t1{clock_index}; % [cite: 767]
            ...
            if epoch_overlap
                ngrid_props = current_doc.document_properties.ngrid; % [cite: 8724]
                spectrogram_props = current_doc.document_properties.spectrogram; % [cite: 8724]

                % --- Read Spectrogram Data ---
                binary_doc_obj = []; % Initialize
                try
                    binary_doc_obj = S.database_openbinarydoc(current_doc, 'spectrogram_results.ngrid'); % [cite: 8728]
                    spectrogram_data_full = mlt.readngrid(binary_doc_obj, ngrid_props.data_dim, ngrid_props.data_type); % [cite: 8725, 8726]
                ...
                time_dim = spectrogram_props.timestamp_ngrid_dim; % [cite: 8730]
                ...
                time_coords_local = ngrid_props.coordinates(coords_start_index_time:coords_end_index_time); % [cite: 8727]

                % --- Extract Frequency Coordinates from NGRID ---
                freq_dim = spectrogram_props.frequency_ngrid_dim; % [cite: 8729]
                ...
                f_vec = ngrid_props.coordinates(coords_start_index_freq:coords_end_index_freq); % [cite: 8727]


                % --- Convert Time Coordinates ---
                try
                    % Create timereference for the source time (local time of the epoch)
                    timeref_in = ndi.time.timereference(e, ndi.time.clocktype('dev_local_time'), doc_epoch_id, 0); % [cite: 6776, 6777, 6778, 6615]
                    
                    % Convert to exp_global_time (datenum format)
                    [time_coords_global_datenum, ~, msg] = S.syncgraph.time_convert(timeref_in, time_coords_local, e, ndi.time.clocktype('exp_global_time')); % [cite: 6700, 6610]
